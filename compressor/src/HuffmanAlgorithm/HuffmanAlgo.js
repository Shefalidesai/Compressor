...
class Codec {  
	//to traverse a Huffman tree and generate binary codes for each symbol present in the tree
	dfs(node, current_code) {
		//is leaf node
		if (typeof (node[1]) === "string") {
			// alternate way
			this.codes[node[1]] = current_code;
			return;
		}

		// for left 
		this.dfs(node[1][0], current_code + '0');
		// for right
		this.dfs(node[1][1], current_code + '1');
	}
